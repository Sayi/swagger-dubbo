...
public class DubboHttpController {
	...
	public ResponseEntity<String> invokeDubbo(@PathVariable("interfaceClass") String interfaceClass,
		...
		Entry<Class<?>, Object> entry = getRef(interfaceMapRef, interfaceClass);
		...
		if (null != interfaceMethods && interfaceMethods.length > 0) {
			Method[] refMethods = httpMatch.findRefMethods(interfaceMethods, operationId,
					request.getMethod());
			method = httpMatch.matchRefMethod(refMethods, methodName,
	 ...
	 * @Description 描述
	 * @author huangjinkai
	 * @param interfaceMapRef
	 * @since string 从哪个版本开始支持
	 * @param interfaceClass
	 * @return
	 */
	private Entry<Class<?>, Object> getRef(Map<Class<?>, Object> interfaceMapRef,
			String interfaceClass) {
		for (Entry<Class<?>, Object> entry : interfaceMapRef.entrySet()) {
	...
	private Object suggestPrameterValue(Type type, Class<?> cls, String parameter)
		...
		// ParameterizedType paraTypr = (ParameterizedType) type;
		...
		if (null != fromType) {
			...
			try {
				...
				// return Json.mapper().readValue(parameter, ((ParameterizedType) type));
			} catch (Exception e) {
				throw new IllegalArgumentException("The parameter value [" + parameter
						+ "] should be json of [" + cls.getName() + "] Type.", e);
			}
		}
		try {
